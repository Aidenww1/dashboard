...
- [x] 🟡 **Automated tests**: `npm test` (tests/run.js auto-runs *.test.js) = **124 asserts / 7 suites**, all green: dates/DST (23), lifeos life-score+readiness (11), finance CSV parse (31), cloudsync per-key merge/reconciliation (20), auth.js client contract (14), operator action-parser security gate (11), backup/restore round-trip (14). Covers the plan's deterministic priority list. Remaining gaps (low value / gated): TDEE (not arg-pure), live RLS behavior (owner cutover), briefing (browser IIFE — verified in-browser instead).
