...
- [~] 🔴 **Multi-device sync = last-write-wins → silent data loss.** Two devices editing `app_state` clobber each other. MOSTLY DONE — cloudsync.js (LifeOSSync) does per-key timestamp merge; 14 pages migrated (health, nutrition, skin, mood, habits, reminders, water, glowlab, library, social, travel, tasks, calendar×2) (c6d9aee, 1ce6c3f). STILL TODO: finance/index/gym (kept their Supabase-Realtime + edit-deferral sync — migrate once the engine gains realtime+edit-defer); whole-key deletion not yet propagated (needs tombstones).
